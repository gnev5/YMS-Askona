# Инструкция по миграции базы данных

## 🚀 Внедрение изменений в структуру БД

### Что было исправлено:

1. **TimeSlot теперь привязаны к конкретным датам** вместо day_of_week
2. **Убрано дублирование записей** - одна запись = одна перевозка
3. **Добавлена связь многие-ко-многим** между записями и слотами
4. **Добавлен журнал слотов** с возможностью точечного управления
5. **Генерация слотов на 4 недели вперед** по умолчанию

### Новые возможности:

- ✅ Журнал слотов с фильтрацией
- ✅ Точечное управление слотами (включить/отключить)
- ✅ Генерация слотов на произвольный период
- ✅ Улучшенная система записей без дублирования
- ✅ Сохранение связей при изменении расписания

## 📋 Пошаговая инструкция по миграции:

### Вариант 1: Полная пересборка (рекомендуется)
```bash
# Запустить скрипт автоматической пересборки
python rebuild_database.py
```

### Вариант 2: Миграция существующей БД
```bash
# Шаг 1: Остановить сервисы
docker compose down

# Шаг 2: Запустить сервисы
docker compose up -d

# Шаг 3: Выполнить миграцию существующей БД
docker compose exec backend python -m app.migrate_existing_db

# Шаг 4: Заполнить начальными данными
docker compose exec backend python -m app.seed
```

### Вариант 3: Ручная пересборка
```bash
# Шаг 1: Остановить сервисы
docker compose down

# Шаг 2: Удалить том с данными
docker volume rm yms-askona_db_data

# Шаг 3: Запустить сервисы
docker compose up -d

# Шаг 4: Выполнить миграцию
docker compose exec backend python -m app.migrate_database

# Шаг 5: Заполнить начальными данными
docker compose exec backend python -m app.seed
```

### Проверка результата
```bash
# Проверить структуру таблицы time_slots
docker compose exec db psql -U yms -d yms -c "\d time_slots"

# Проверить данные
docker compose exec db psql -U yms -d yms -c "SELECT COUNT(*) FROM time_slots;"
docker compose exec db psql -U yms -d yms -c "SELECT COUNT(*) FROM bookings;"
docker compose exec db psql -U yms -d yms -c "SELECT COUNT(*) FROM booking_time_slots;"

# Проверить, что поле slot_date существует
docker compose exec db psql -U yms -d yms -c "SELECT slot_date, start_time, end_time FROM time_slots LIMIT 5;"
```

## 🔧 Новые API endpoints:

### Управление слотами:
- `GET /api/time-slots/journal` - журнал слотов с фильтрацией
- `POST /api/time-slots/generate` - генерация слотов на период
- `PUT /api/time-slots/{id}/availability` - включить/отключить слот
- `DELETE /api/time-slots/{id}` - удалить конкретный слот

### Управление записями:
- `POST /api/bookings` - создание записи (упрощенное)
- `GET /api/bookings/{id}/slots` - получить слоты записи
- `PUT /api/bookings/{id}/cancel` - отменить запись
- `DELETE /api/bookings/{id}` - удалить запись

## ⚠️ Важные изменения:

1. **Структура TimeSlot изменилась:**
   - Убрано поле `day_of_week`
   - Добавлено поле `slot_date` (конкретная дата)
   - Добавлены поля `is_available`, `created_at`, `updated_at`

2. **Структура Booking изменилась:**
   - Убраны поля `time_slot_id`, `booking_date`, `group_id`
   - Добавлено поле `status`
   - Добавлены поля `created_at`, `updated_at`

3. **Новая таблица BookingTimeSlot:**
   - Связь многие-ко-многим между Booking и TimeSlot
   - Поля: `booking_id`, `time_slot_id`

## 🧪 Тестирование:

После миграции проверьте:
1. Календарь отображает слоты корректно
2. Создание записей работает
3. Журнал слотов доступен
4. Генерация слотов работает
5. Отмена записей работает

## 🔄 Откат (если что-то пошло не так):

Если нужно откатить изменения:
```bash
# Остановить сервисы
docker compose down

# Удалить том с данными
docker volume rm yms-askona_db_data

# Запустить заново
docker compose up --build -d

# Заполнить данными
docker compose exec backend python -m app.seed
```

## 📊 Результат миграции:

- ✅ Исправлены критические проблемы структуры БД
- ✅ Записи больше не теряются при изменении расписания
- ✅ Убрано дублирование записей
- ✅ Добавлен журнал слотов
- ✅ Улучшена гибкость системы
- ✅ Готовность к масштабированию
