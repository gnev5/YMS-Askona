...
const BookingOut: React.FC = () => {
  ...
  const formatBookingTooltip = (bookings?: SlotBookingInfo[]) => {
    ...
    const lines = bookings.map((b, idx) => {
      ...
      const volume = b.cubes ?? 'Не указано'
      return `${idx + 1}. ТС: ${transportSheet} · Поставщик: ${supplier} · Объем: ${volume}`
    })
    return lines.join('\n')
  }
    ...
    <div>
      ...
      <form onSubmit={handleImport} style={{ marginBottom: 12, padding: 12, border: '1px dashed #d1d5db', borderRadius: 8 }}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button type="submit" disabled={importLoading}>{importLoading ? 'Импорт...' : 'Импорт Excel'}</button>
        </div>
        {importResult && (
